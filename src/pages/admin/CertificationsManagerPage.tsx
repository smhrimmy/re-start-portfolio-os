  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 text-gray-100 font-sans pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {items.map(cert => (
          <div key={cert.id} className="p-5 rounded-2xl bg-[#f4f4f3] border border-[#dcdcdc] space-y-3 shadow-2xs hover:border-black/20 transition-colors flex flex-col justify-between">
            <div className="space-y-2">
              <div className="flex items-start justify-between">
                <span className="p-2 rounded-xl bg-amber-500/15 text-amber-800 border border-amber-500/20">
                  <ShieldCheck className="w-4 h-4" />
                </span>
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => handleStartEdit(cert)}
                    className="p-1.5 text-[#55555e] hover:text-[#1a1a1a] hover:bg-black/5 rounded-lg transition-colors"
                  >
                    <Edit2 className="w-3.5 h-3.5" />
                  </button>
                  <button
                    onClick={() => handleDelete(cert.id)}
                    className="p-1.5 text-[#55555e] hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors"
                  >
              ...
              <div>
                <h3 className="text-sm font-bold text-[#1a1a1a] leading-snug">{cert.name}</h3>
                <p className="text-xs text-[#ad314d] font-semibold mt-0.5">{cert.issuer}</p>
              </div>

              <div className="flex items-center gap-3 text-[11px] font-mono text-[#55555e] pt-1">
                <span className="flex items-center gap-1"><Calendar className="w-3 h-3 text-[#777780]" /> {cert.issueDate}</span>
            ...
            {cert.credentialUrl && (
              <div className="pt-2 border-t border-black/8">
                <a
                  ...
                  className="text-xs text-[#1a1a1a] hover:text-[#ad314d] font-semibold inline-flex items-center gap-1"
                >
          ...
          <div className="bg-white border border-[#dcdcdc] rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between pb-3 border-b border-black/8">
              ...
              <button
                ...
                className="text-[#55555e] hover:text-[#1a1a1a] p-1 rounded-lg"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
