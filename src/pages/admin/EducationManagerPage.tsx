  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 text-gray-100 font-sans pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {items.map(edu => (
          <div key={edu.id} className="p-5 rounded-2xl bg-[#f4f4f3] border border-[#dcdcdc] space-y-3 shadow-2xs hover:border-black/20 transition-colors">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-base font-bold text-[#1a1a1a]">{edu.degree}</h3>
                <p className="text-xs text-[#ad314d] font-semibold flex items-center gap-1.5 mt-0.5">
            ...
            <div className="flex flex-wrap items-center gap-3 text-[11px] font-mono text-[#55555e]">
              <span className="flex items-center gap-1"><Calendar className="w-3 h-3 text-[#777780]" /> {edu.year}</span>
              <span>·</span>
              <span className="flex items-center gap-1"><MapPin className="w-3 h-3 text-[#777780]" /> {edu.location}</span>
              {edu.score && (
                <>
                  <span>·</span>
                  <span className="flex items-center gap-1 text-emerald-800 font-bold"><Award className="w-3 h-3 text-emerald-700" /> {edu.score}</span>
                </>
              )}
            </div>
